...
# +++ 飛行物クラス +++
class Flying_thing:
    def __init__(self, x, y, bank_x, bank_y, width, height, mode,  bank_no=0, 
...
class Game:
    ...
    def reset(self):
        ...
        # 飛行物リストの設定
        self.flying_list = [
            Flying_thing(pyxel.width, 60, 16, 16, 17, 6, "w"),  # 彗星
            Flying_thing(pyxel.width, 60, 32, 16, 16, 16, "w"),  # 人工衛星
            Flying_thing(pyxel.width, 60, 16, 32, 16, 16, "w"),  # 地球
            Flying_thing(pyxel.width, 60, 34, 37, 13, 5, "w"),  # 黒板消し
            Flying_thing(pyxel.width, 60, 0, 16, 16, 7, "w"), # UFO
            Flying_thing(pyxel.width, 60, 48, 0, 16, 13, "w")  # ウシ
        ]
        ...
        self.flying_thing_fusei = Flying_thing(pyxel.width, 60, 35, 65, 41, 31, "w")
    ...
    def is_collision_circle(self, x_1, y_1, width_1, height_1, x_2, y_2, width_2, height_2):
        ...
        r_1 = max(width_1, height_1) / 2 * 0.8
        r_2 = max(width_2, height_2) / 2 * 0.8
   ...
   # ** 障害物（敵列）の処理 **
    def update_play_shogai_butu(self):
        ...
        if self.spawn_interval <= 0:
            ...
            self.spawn_interval = 101 - self.level * 8.5
        else:
    ...
    def update_play_flying_thing(self):
        ...
        if self.is_destroyed:
            ...
            self.fl_time_counter = 300  # 10秒待つ
            # 次の飛行物インデックスを進める（ループ）
   ...
   # ** ミサイルとの衝突判定 ** 
    ...
    def draw_title(self):
        ...
        # high_x,high_y = 100, 15
        # score_x = 120
        # space_x, space_y = 10, 8

        # pyxel.text(high_x, high_y + space_y * 1, "1st", 10)
        # pyxel.text(high_x, high_y + space_y * 2, "2nd", 7)
        # pyxel.text(high_x, high_y + space_y * 3, "3rd", 7)
        # pyxel.text(high_x, high_y + space_y * 4, "4th", 7)
        # pyxel.text(high_x, high_y + space_y * 5, "5th", 7)

        # pyxel.text(score_x, high_y + space_y * 1, "1900", 10)
        # pyxel.text(score_x, high_y + space_y * 2, "2900", 7)
        # pyxel.text(score_x, high_y + space_y * 3, "3900", 7)
        # pyxel.text(score_x, high_y + space_y * 4, "4900", 7)
        # pyxel.text(score_x, high_y + space_y * 5, "5900", 7)
